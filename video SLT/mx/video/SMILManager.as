class mx.video.SMILManager
{
    ...
    null[] = (Error)() == null ? (null, throw , function (success)
    {
        try
        {
            ...
            else
            {
                ...
                videoTags = new Array();
                var _loc4 = xml.firstChild;
                if (_loc4.nodeName == null)
                {
                ...
                else if (_loc4.nodeName.toLowerCase() != "smil")
                {
                    throw new mx.video.VideoError(mx.video.VideoError.INVALID_XML, "URL: \"" + _url + "\" Root node not smil: " + _loc4.nodeName);
                } // end else if
                var _loc5 = false;
                for (var _loc3 = 0; _loc3 < _loc4.childNodes.length; ++_loc3)
                {
                    ...
                    if (_loc2.nodeName.toLowerCase() == "head")
                    {
                        this.parseHead(_loc2);
                        continue;
                    } // end if
                    if (_loc2.nodeName.toLowerCase() == "body")
                    {
                        _loc5 = true;
                        this.parseBody(_loc2);
                        continue;
                    } // end if
                    throw new mx.video.VideoError(mx.video.VideoError.INVALID_XML, "URL: \"" + _url + "\" Tag " + _loc2.nodeName + " not supported in " + _loc4.nodeName + " tag.");
                } // end of for
                if (!_loc5)
                {
    ...
    function parseVideo(node)
    {
        ...
        for (var _loc4 in node.attributes)
        {
            ...
            {
                _loc3.dur = Number(node.attributes[_loc4]);
                continue;
            } // end if
        } // end of for...in
